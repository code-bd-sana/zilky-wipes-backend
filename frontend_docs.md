...
- Call `POST /api/v1/orders` with the user's cart items, shipping details, and the selected `shippingMethodId`.
- **Response:** The backend will securely calculate the final shipping cost based on the active rules engine, reserve stock, generate a Stripe Checkout Session, and return a `checkoutUrl`.
...
## 6. Dynamic Shipping Flow

The platform now uses a dynamic shipping rules engine. Shipping costs are calculated based on cart totals, weights, item counts, and the user's delivery address.

**Step 1: Estimate Shipping (Cart / Checkout Page)**

- When the user views their cart or enters their shipping address, call `POST /api/v1/shipping/estimate`.
- **Payload Example:**
  ```json
  {
    "items": [
      { "productVariantId": "uuid-1234", "quantity": 2, "isSubscription": false }
    ],
    "country": "US",
    "state": "CA",
    "zipCode": "90210"
  ...
  ```
- **Response:** Returns a list of available `methods` and a marketing `message` (e.g., "You're only $5.00 away from FREE Shipping").
  ```json
  {
    "methods": [
      {
    ...
    "message": "You're only $5.00 away from FREE Shipping."
  }
  ```

**Step 2: Display Options**

- Display the returned `methods` to the user so they can select their preferred shipping option.

**Step 3: Submit Order**

- Once the user selects a method, include the `shippingMethodId` in your `POST /api/v1/orders` payload.
- _Note:_ You do not need to send the exact `shippingCost` in the order payload anymore (it is optional), as the backend will securely recalculate the exact cost using the Rules Engine to prevent tampering.
...
- **Shipping Estimate:** `POST /api/v1/shipping/estimate`
